...
class Driver(Node):
    ...
    def camera_callback(self, data):
        mpl.use('TkAgg')
        bird, BCB = self.image_tranform(data, cv2.COLOR_BGR2RGB, 2, 130, 200)
        ...
        right_line = self.draw_line(40 , 40 , 40 , BCB , 1)
        #left_line = self.draw_line(40 , 40 , 40 , BCB , 0)
        ...
        if self.steer >= 50:
            ...
            self.distance = 150
        right_x , right_y = self.line_to_go(self.distance , right_line , 1)
        ...
        if sys.argv[1] == "show":
            ...
            for i in range(len(right_line) - 1):
                plt.plot([right_line[i][0], right_line[i+1][0]], [right_line[i][1], right_line[i+1][1]], color="blue")
            plt.plot([1000, np.average(x)], [2000, np.average(y)], color="yellow")
            plt.show()
            ...
            #     plt.plot([left_line[i][0], left_line[i+1][0]], [left_line[i][1], left_line[i+1][1]], color="pink")
            # plt.plot([1000, np.average(x)], [2000, np.average(y)], color="yellow")
            plt.show()
    ...
    def image_tranform(self, data, ch, color, thr_min, thr_max):
        ...
        src = np.float32([[838, 643.3], [675.5, 773.3], [1115, 773.3], [1037, 643.3]])
        ...
        dst = np.float32([[850, 0], [850, 2000], [1150, 2000], [1150, 0]])
        M = cv2.getPerspectiveTransform(src, dst)
        aerial_view = cv2.warpPerspective(image, M, (2000, 2000))
    ...
    def draw_line(self , block_h  , lane_range , block_count , BCB , side):
        ...
        histogram = None
        if side == 0:
            histogram = np.sum(BCB[int(BCB.shape[0] * 0.8):,:], axis=0)
        else:
        ...
        lefts_base = np.argmax(histogram[:midpoint])
